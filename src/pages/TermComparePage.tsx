...
interface TermComparePageProps {
  onBookCall: () => void;
}

const TermComparePage: React.FC<TermComparePageProps> = ({ onBookCall }) => {
  ...
  useEffect(() => {
    fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:3000'}/api/policies?type=Term`)
      .then(r => r.json())
      .then(data => {
        setPolicies(data);
  ...
  const renderPolicyCard = (policy: Policy | undefined, side: 'left' | 'right') => {
    const color = side === 'left' ? '#4f46e5' : '#8b5cf6'; // using indigos and purples for term/life
    if (!policy) return (
  ...
  return (
    <div style={{ maxWidth: '1100px', margin: '0 auto', padding: '3rem 1.5rem' }}>
      <div style={{ textAlign: 'center', marginBottom: '2.5rem' }}>
        <h1 style={{ fontSize: '2rem', color: '#1f2937', fontWeight: 700, marginBottom: '0.5rem' }}>Compare Life Insurance Plans</h1>
      ...
      <div style={{ display: 'flex', gap: '1rem', marginBottom: '2rem', alignItems: 'center' }}>
        <select
          ...
          style={{ flex: 1, padding: '0.75rem 1rem', borderRadius: '8px', border: '2px solid #4f46e5', outline: 'none', fontSize: '0.95rem', fontWeight: 500, color: '#1f2937' }}
        >
        ...
        <select
          ...
          style={{ flex: 1, padding: '0.75rem 1rem', borderRadius: '8px', border: '2px solid #8b5cf6', outline: 'none', fontSize: '0.95rem', fontWeight: 500, color: '#1f2937' }}
        >
      ...
      {loading ? (
        ...
        <div style={{ textAlign: 'center', padding: '4rem', color: '#6b7280' }}>
          ...
          <p>No life insurance plans added yet. Ask your admin to add plans.</p>
        </div>
      ) : (
        /* Compare Box */
...
export default TermComparePage;
